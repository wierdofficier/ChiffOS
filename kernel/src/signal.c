...
void handle_signal(struct task * proc, signal_t * sig) {
...
void return_from_signal_handler(void) {
	...
	//spin_lock(sig_lock);
	list_insert(rets_from_sig, (struct task *)current_task);
...
int send_signal(pid_t process, uint32_t signal) {
	struct task * receiver = process_from_pid(process);
