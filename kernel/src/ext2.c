...
static void mkdir_ext2(fs_node_t * parent, char * name, uint16_t permission) {
	...
	/* Assign it to root */
	inode->uid = current_task->user; /* user */
	inode->gid = current_task->user;
...
static fs_node_t * mount_ext2(fs_node_t * block_device, int flags) {
	...
	debug_print(NOTICE, "Mounting ext2 file system...1");
	ext2_fs_t * this = valloc(sizeof(ext2_fs_t));

	memset(this, 0x00, sizeof(ext2_fs_t));

	this->flags = flags;
debug_print(NOTICE, "Mounting ext2 file system...2");
	this->block_device = block_device;
	this->block_size = 1024;
this->block_device->refcount = -1;
	//vfs_lock(this->block_device);
debug_print(NOTICE, "Mounting ext2 file system...3");
...
debug_print(NOTICE, "Mounting ext2 file system...4");
	this->inode_size = SB->inode_size;
	if (SB->inode_size == 0) {
		this->inode_size = 128;
	}
debug_print(NOTICE, "Mounting ext2 file system...5");
...
fs_node_t * ext2_fs_mount(char * device, char * mount_path) {
	...
	fs_node_t * fs = mount_ext2(fnodeGLOBAL, flags);
