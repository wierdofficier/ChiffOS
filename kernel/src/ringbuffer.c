...
void ring_buffer_select_wait(ring_buffer_t * ring_buffer, void * process) {
	...
	list_insert(((	struct task *)process)->node_waits, ring_buffer);
}

size_t ring_buffer_read(ring_buffer_t * ring_buffer, size_t size, unsigned char * buffer) {
	size_t collected = 0;
	while (collected == 0) {
...
size_t ring_buffer_write(ring_buffer_t * ring_buffer, size_t size, unsigned char  * buffer) {
	size_t written = 0;
	while (written < size) {
	//	spin_lock(ring_buffer->lock);
