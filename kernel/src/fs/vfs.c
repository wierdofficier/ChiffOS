...
/**
 * selectwait_fs: Inform a node that it should alert the current_task.
 */
...
int create_file_fs(char *name, uint16_t permission) {
	fs_node_t * parent;
	char *cwd = (char *)(current_task->wd_name);
...
void vfs_install(void) {
	...
	struct vfs_entry * root = valloc(sizeof(struct vfs_entry));
...
int vfs_mount_type(char * type, char * arg, char * mountpoint) {
	...
	fs_node_t * n = t("ext2", mountpoint); //denna monterar ext2
	...
	if (node && node->value) {
		...
		ent->fs_type = strdup("ext2");
		ent->device  = strdup("/dev/hda");
	}

	debug_print(NOTICE, "Mounted %s[%s] to %s: 0x%x", "ext2", "/dev/hda", mountpoint, n);
	...
	if (*i == '\0') {
		...
		while (1) {
			...
			if (!found) {
				...
				struct vfs_entry * ent = valloc(sizeof(struct vfs_entry));
				ent->name = strdup(at);
				ent->file = NULL;
				ent->device = NULL;
				ent->fs_type = NULL;
...
fs_node_t *get_mount_point(char * path, unsigned int path_depth, char **outpath, unsigned int * outdepth) {
	...
	*outdepth = _tree_depth;
	...
	if (last) {
		fs_node_t * last_clone = valloc(sizeof(fs_node_t));
