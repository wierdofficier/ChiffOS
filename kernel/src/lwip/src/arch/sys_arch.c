...
create_task_thread((void*)thread,PRIO_HIGH);

	//create_kernel_tasklet(thread, "[tcpip]", NULL);
	//err = create_kernel_task_on_core(&id, (entry_point_t)thread, arg, prio, boot_processor);
	debug_print(NOTICE,"sys_thread_new: create_kernel_task err %d, id = %u, prio = %d\n", err, id, prio);
...
inline   int sem_init(sem_t* s, unsigned int v)
{
	unsigned int i;
...
u32_t sys_arch_mbox_fetch(sys_mbox_t * mbox, void **msg, u32_t timeout)
{
	...
	if ( err == 0)
		return 0;
 //LWIP_DEBUGF(SYS_DEBUG, ("SYS_ARCH_TIMEOUT: %d\n", err));
	return SYS_ARCH_TIMEOUT;
}
...
int rand__(void) // RAND_MAX assumed to be 32767
{
...
void sys_arch_netconn_sem_alloc(void)
{
	...
	sem = netconn_sem = (sys_sem_t*) malloc(sizeof(sys_sem_t));
