...
static uintptr_t proc_sbrk(uint32_t num_pages, struct task * proc) {
...
static void * map_in (shm_chunk_t * chunk, struct task * proc) {
	if (!chunk) {
		return NULL;
	}
...
void * shm_obtain (char * path, size_t * size) {
	//spin_lock(bsl);
	struct task * proc = (struct task *)current_task;
...
void shm_release_all (struct task * proc) {
	//spin_lock(bsl);
