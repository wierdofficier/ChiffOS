...
int failures =0 ;
 ...
 void panic_assert(const char *file, unsigned int line, const char *desc) {
	printk(  "Assertion failure: %s:%d: %s\n", file, line, desc);
	failures++;
}
...
u32 placement_address_new = (u32)&end; //from linker script
...
static void * __attribute__ ((malloc)) klmalloc(uintptr_t size);
  void * __attribute__ ((malloc)) klrealloc(void * ptr, uintptr_t size);
static void * __attribute__ ((malloc)) klcalloc(uintptr_t nmemb, uintptr_t size);
  void * __attribute__ ((malloc)) klvalloc(uintptr_t size);
...
void * __attribute__ ((malloc)) malloc_(uintptr_t size) {
	//spin_lock(mem_lock);
	return valloc(size);
}
...
void * __attribute__ ((malloc)) malloc_a(uintptr_t size) {
	//spin_lock(mem_lock);
	return valloc(size);
}
...
void free(void * ptr) {
	//spin_lock(mem_lock);
	if ((uintptr_t)ptr > placement_address_new) {
		klfree(ptr);
	}
	//spin_unlock(mem_lock);
}
